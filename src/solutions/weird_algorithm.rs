...
pub fn solve(mut n: u64) -> Result<Vec<u64>, CsesError> {
    if !(VALID_RANGE).contains(&n) {
        let msg = format!("n is {} but expected {} ≤ n ≤ {}", n, VALID_RANGE.start(), VALID_RANGE.end());
...
mod tests {
    ...
    fn unit_0() {
        let n: u64 =0;
        let expected: CsesError = CsesError::InvalidInput("n is 0 but expected 1 ≤ n ≤ 1000000".to_string());
        let result= solve(n);
    ...
    fn unit_1_000_001() {
        let n: u64 =1_000_001;
        let expected: CsesError = CsesError::InvalidInput("n is 1000001 but expected 1 ≤ n ≤ 1000000".to_string());
        let result= solve(n);
        assert_eq!(result, Err(expected));
    }
